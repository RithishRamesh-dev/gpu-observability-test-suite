...
def fill_memory_stress(device: torch.device, fill_percent: float, duration_sec: int):
    ...
    # Stress memory bandwidth using only the already-allocated tensor.
    # Use scalar in-place ops so we don't need a second tensor of the same size.
    start = time.monotonic()
    iters = 0
    logger.info("Holding allocation and running memory bandwidth stress for %ds...", duration_sec)
    while time.monotonic() - start < duration_sec:
        big_tensor.add_(0.0001)   # Read + write entire tensor in place
        if iters % 50 == 0:
